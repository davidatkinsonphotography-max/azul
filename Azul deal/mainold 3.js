...
function resetOneboxPosition() {
  ...
  function highlightCurrentPlayer() {
    playerNames.forEach((el) => el.classList.remove('nameHighlight'));
    const ticked = getTickedPlayerIndexes();
    const current = ticked[currentPlayerIndex];
    if(current !== undefined) {
      playerNames[current].classList.add('nameHighlight');
    }
  }
  ...
  tiles.forEach(tile => {
    tile.onclick = () => {
      ...
      setTimeout(() => {
        ...
        relayoutExcess();
        advanceToNextPlayer(); // ✅ Moved here to avoid double trigger
      }, 900);
    };
  });
}
